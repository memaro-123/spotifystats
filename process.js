...
jsonStream.on("data", ( {value} ) => {
  console.log(`Processing ${value.master_metadata_track_name}`)
    if(value.ts.startsWith(year)){// year selector
...
jsonStream.on("end", () => {
    ...
    const topSongs = songs.slice(0, 200);
    const topAlbums = albums.slice(0, 200);
