...
const App = () => {
  ...
  const handleFormSubmit = (event) => {
    ...
    musicService.submitFormData(files)
      .then((data) => {
  ...
  const handleYearClick = (event) => {
    event.preventDefault()
    if (!(Number.isFinite(Number(year)) && (Number(year) >= 2006 && Number(year) <= 2025)) && (year !== "all" && year !== "All")) {
  ...
  return (
    <div>
      ...
      {dataStatus ? <YearSearch year={year} onYearChange={handleYearChange} onYearClick={handleYearClick} /> : null}
      <FileUpload redErrorText={redErrorText} handleFormSubmit={handleFormSubmit} fileInputRef={fileInputRef} />
      {dataStatus ? <Lists sayYear={sayYear} first10Ar={first10Ar} first10Al={first10Al} first10So={first10So} /> : null}
    </div>
  )
}
